...
Key Performance Metrics (KPMs) and other relevant metrics will be measured on a regular cadence (daily, if possible) and published to the DM’s metric monitoring system (currently SQuaSH).
The SQuaRE scientist will monitor the metric measurements and present regressions in a timely fashion to the SST with an initial assesment of potential causes for the regression.
The SST will propose an appropriate team to investigate the regression to the DM Project Manager.
The SQuaRE scientist will report progress to the SST and the SST may guide further investigation and resolution.
