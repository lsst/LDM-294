...
\item Membership
	\begin{itemize}
        \item Core members:
	        ...
	        \item Chaired by the Systems Engineer (\secref{role:sysengineer}).
                ...
                \end{itemize}
        \item Optional members, involved when topics to discuss are relevant for their areas of expertise:
                \begin{itemize}
                ...
                \item T/CAMs, they can delegate to their deputies
                \end{itemize}
	...
	\item For on-line virtual meetings, if a consensus or quorum is not reached within one week, the DM Project Manager will make a unilateral decision
        \item DMPM can take unilateral decisions also in case of urgency, i.e. a decision is required immediately and DMCCB is not available. In that case DMCCB will assess the change a posteriori.
	\end{itemize}
\item Responsibilities
	\begin{itemize}
	...
	\item Reviews and approves/rejects proposed changes to baselined items
        \item Overviews the release process, approves new or unplanned releases, i.e. patch releases, and approves release content
        \item Reviews and approves all RFCs prior to 'Adoption'
        \item Monitors DM Jira issues status
	\end{itemize}
\end{itemize}

The DM-CCB will meet, physically or virtually, every week. Agenda will be available beforehand and will include the following discussion topics:

\begin{itemize}
\item Approve the release schedule
        \begin{itemize}
        \item All releases need to be proposed to the DM-CCB via RFC
        \item Once a release has been accepted, the Configuration-Release Manager will create a corresponding release issue
        \end{itemize}
\item Approve release content
        \begin{itemize}
        \item The DM-CCB will monitor, with the support of Optional CCB members, RFCs, epics and stories that are implemented in master repositories and therefor included in major or minor releases
        \item The DM-CCB will approve, with the support of Optional CCB members, all issues requested to be included in a patch release. This information has to be provided in the RFC together with the patch release request.
...
\item Approve any other RFCs, flagging them as 'adopted', before implementing
\end{itemize}
