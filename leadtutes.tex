...
\item Support of publicly software products build infrastructure e.g. Jenkins, and release tools e.g. container creation.

\item Support a repository of binary releases e.g. private conda repository.
...
Leadership of this cross cutting group is based in Tucson see \secref{role:mwlead}.
This is a cross cutting team which requires effort from the Data Facility(ies) pipelines and architecture.
Effort for middleware must be negotiated by the middleware manager with the \gls{T/CAM}s of the various institutes.
The main elements of middleware are \gls{Butler} and Supertask - however it is also important that we have back end implementations which work at scale in the Data Facility for data release production.


\subsubsection{Science Platform \label{sec:sciplat}}
Leadership of this cross cutting group is based in Tucson see \secref{role:lsplead}
The platform consists of three components as outlined in \citeds{LDM-542} and is deployed in
...
and as such a single science lead has been identified as essential (see \secref{role:scip}.
