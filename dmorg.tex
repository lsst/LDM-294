...
The DMCCB is maintaining the release plan \citeds{LDM-564} based on the project milestones.

All releases will be identified using the Jira release functionality or by a release issue.

Any extra release, major, minor or patch, needs to be requested by the end user to the DMCCB using a RFC Jira issue. The RFC shall contain:

\begin{itemize}
\item the reason that justify an extra release;
...
\item the date the release will be available (an estimation, that may change)
\item the list of Jira issues that will be implemented
\end{itemize}
