...
The DM-CCB will provide a release plan on a yearly bases.
...
\item list of proposed functionalities or fixes (Jira issues) which will be implemented in the release
\item date the release is requested to be available and specify the urgency.
\end{itemize}

The DM-CCB will assess the release in one week's time. If the release is urgent, DM-CCB will assess it in 24 hours.

As an outcome of the assessment, DM-CCB shall approve or reject the release.
If the release request is not approved, DM-CCB will provide the reason.
If the release request is approved, DM-CCB shall add a comment to the RFC with the following information:

\begin{itemize}
\item version that will identify the release
\item date the release will available (an estimation, that may change)
\item confirm list of Jira issues that will be implemented
\end{itemize}

The DM-CCB will monitor the progress of the release's activity on a weekly bases.
