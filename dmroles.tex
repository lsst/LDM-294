...
between the Project Manager and deputy (see \secref{role:ddmpm}).
...
There are at least two roles which involve work across institute and WBS boundaries.
These individuals act as TCAM for the cross cutting activity, including holding standups and resolving technical difficulties.
They should provide a schedule for the activities and synchronize it with the other TCAMS.
Day to day management of staff would still reside with the WBS TCAM - stories loaded on individuals would have to be agreed.
Hence this is  a slightly more coordination oriented role but the leads have authority to prioritize stories in the relevant WBS areas.
\subsubsection{LSP Lead} \label{role:lsplead}
The LSST Science Platform spans multiple WBS and bringing together authentication, front end services, database access and notebook execution. Frossie Economou is the lead for the LSP.

\subsubsection{Middleware Lead} \label{role:mwlead}
Middleware covers several WBS elements and requires multiple part of the system to work in unison. This brings together
 task execution  and workflow management , data access (butler), and provenience. Fritz Mueller is the middleware lead.
