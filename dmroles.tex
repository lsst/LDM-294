...
\begin{itemize}
\item Ensure that the scientific goals of the DM system are met
...
\item Overseework on lower level requirements documents
...
\item Direct the Interface Scientist's (\secref{sect:dmis}) work on external ICDs
\end{itemize}
...
\begin{itemize}
  ...
  \item{Synchronize the constuction schedule with development in WBS elements managed by other T/CAMs}
  \item{Maintain the budget for their WBS and ensuring that all work undertaken is charged to the correct accounts}
  \item{Work with the relevant Science Leads and Product Owners (\secref{role:prodo}) to devleop the detailed plan for each cycle and sprint as the occur}
