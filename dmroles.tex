...
\begin{itemize}
  ...
  \item{Synchronize the DM plan, managed as per \secref{sect:plan}, with theLSST PMCS}
...
The product owner for the Platform is the DMSS \secref{role:dmps}, who has final say on requirements and features.
...
With the \gls{Systems Engineering} Team (\secref{sect:sysengt}) the \gls{Systems Engineer} owns the DM entries in the risk register and is generally in charge of the \textit{process} of building DM products.
...
In addition, the \gls{Systems Engineer} is responsible for the process to define \& maintain DM interfaces (internal and external)
...
The \gls{Systems Engineer} shall chair the DM Change Control Board (\secref{sect:dmccb})
...
The \gls{DMIS} is is responsible for all external interfaces to the DM Subsystem. This includes ensuring that appropriate tests for those interfaces are defined. This is a responsibility delegated from the DM Systems Engineering Team (\secref{sect:sysengt}).
...
The Software Architect will work closely with the Systems Engineer Team (\secref{sect:sysengt}) to ensure that processes are in place for tracing requirements to the codebase and providing hooks to ensure that requirement verification is possible.
...
Each Lead Institution (as defined in \secref{sect:leadtutes}; see also \tabref{tab:wbs}) has a \gls{T/CAM} and Scientific or Engineering Lead, who jointly have overall responsibility for a broad area of DM work, typically a \gls{Work Breakdown Structure} (\gls{WBS}) Level 2 element. They are supervisors of the team at their institution, with roles broadly analogous to those of the \gls{DMPM} and \gls{Subsystem Scientist}.
...
\begin{itemize}
  ...
  \item{Work with the DM Project Controller (\secref{role:pcon}) to ensure that all plans and milestones are captured in theLSST Project Controls system}
...
The Institutional Science/Engineering Leads serve as product owners (\secref{role:prodo}) for the major components of the DM System (\gls{Alert Production}, Data \gls{Release} Production, Science User Interface etc).
...
Institutional science leads are members of the DM System Science Team (\secref{sect:dmsst}) and, as such, report to the \gls{DMSS} (\secref{role:dmps}).

\subsection{DM Science \gls{Validation} Scientist}
\label{role:dmsvs}

The DM Science \gls{Validation} Scientist leads the Science \gls{Validation} team (\secref{sect:dmsvt}).
