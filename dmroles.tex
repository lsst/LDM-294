...
These individuals act coordinators for the cross cutting activity, including holding standups and resolving technical difficulties.
They should provide a schedule for the activities and synchronize it with the other TCAMS.
Day to day management of staff resides with the WBS TCAM - stories loaded on individuals have to be agreed the the relevant TCAM.
Though  this is  more a coordination oriented role the managers  have authority to prioritize stories in the relevant WBS areas.
\subsubsection{LSP manager} \label{role:lsplead}
The LSST Science Platform spans multiple WBS elements  bringing together authentication, front end services, database access and notebook execution. Frossie Economou is the manager for the LSP.

\subsubsection{Middleware manager} \label{role:mwlead}
Middleware covers several WBS elements and requires multiple parts of the system to work in unison. This brings together
 task execution, workflow management , data access (butler), and provenience. Fritz Mueller is the middleware manager.
