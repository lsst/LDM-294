...
\usepackage[paperwidth=24cm,paperheight=208.12cm,
...
\node (BUTLERpkg) [tbox,below=3mm of BUTLER.north] {{\tiny \color{gray} \begin{verbatim} daf_persistence/db/daf_fmt_* \end{verbatim} }  }; 
 ...
 \node (DAC) [pbox,below=4pt of BUTLER] {{\tiny \color{gray}.} \newline 
...
\textbf{Alert DB}
 ...
 \node (DAX) [pbox,below=4pt of DACRM] {{\tiny \color{gray}.} \newline 
\textbf{DAX Web Services}
 }; 
...
\node (DAXWEBpkg) [tbox,below=3mm of DAXWEB.north] {{\tiny \color{gray} \begin{verbatim} dax_webserv/dax_webservcommon \end{verbatim} }  }; 
 ...
 \node (L1DB) [pbox,below=136pt of DAX] {{\tiny \color{gray}1.02C.06.01.01} \newline 
\textbf{L1 Catalog DB}
 }; 
...
\node (L2DBpkg) [tbox,below=3mm of L2DB.north] {{\tiny \color{gray} \begin{verbatim} cat \end{verbatim} }  }; 
 ...
 \node (DEVEL) [pbox,below=340pt of DAC] {{\tiny \color{gray}.} \newline 
...
\node (CIpkg) [tbox,below=3mm of CI.north] {{\tiny \color{gray} \begin{verbatim} sconsUtils/base/lsstsw/lsst_build \end{verbatim} }  }; 
...
\textbf{Infrastructure}
 ...
 \node (DBB) [pbox,below=4pt of BPS] {{\tiny \color{gray}.} \newline 
\textbf{Data Backbone}
 }; 
...
\textbf{Provenance DB}
 ...
 \node (INFRASYS) [pbox,below=68pt of DBB] {\textbf{Infra Systems}
 }; 
...
\node (QSERVpkg) [tbox,below=3mm of QSERV.north] {{\tiny \color{gray} \begin{verbatim} qserv/partition/scisql \end{verbatim} }  }; 
 ...
 \node (INTGTEST) [pbox,below=170pt of INFRA] {\textbf{Integration Test}
 }; 
 \draw[pline] (DM.east) -| ++(0.4,0)  |- (INTGTEST.west);
 \node (ITENV) [pbox,below=4pt of INTGTEST] {{\tiny \color{gray}.} \newline 
...
\textbf{Archive IT}
 ...
 \node (ITBASE) [pbox,below=272pt of ITARC] {{\tiny \color{gray}.} \newline 
\textbf{Base IT}
 }; 
...
\textbf{Level 1 Services}
 ...
 \node (ARCAT) [pbox,right=15mm of L1SRV] {\textbf{Aux Tel Archiver}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (ARCAT.west);
 \node (ARCCOM) [pbox,below=4pt of ARCAT] {\textbf{ComCam Archiver}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (ARCCOM.west);
 \node (ARCLSST) [pbox,below=4pt of ARCCOM] {\textbf{LSSTCam Archiver}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (ARCLSST.west);
 \node (CARCCOM) [pbox,below=4pt of ARCLSST] {\textbf{ComCam Catchup}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (CARCCOM.west);
 \node (CARCLSST) [pbox,below=4pt of CARCCOM] {\textbf{LSSTCam Catchup}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (CARCLSST.west);
 \node (PRMPTCOM) [pbox,below=4pt of CARCLSST] {\textbf{ComCam Prompt}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (PRMPTCOM.west);
 \node (PRMPTLSST) [pbox,below=4pt of PRMPTCOM] {\textbf{LSSTCam Prompt}
 }; 
 \draw[pline] (L1SRV.east) -| ++(0.4,0)  |- (PRMPTLSST.west);
 \node (MINIBRKR) [pbox,below=204pt of L1SRV] {{\tiny \color{gray}1.02C.03.03} \newline 
\textbf{Alert Mini-Broker}
 }; 
 \draw[pline] (L1.east) -| ++(0.4,0)  |- (MINIBRKR.west);
 \node (L2) [pbox,below=986pt of L1] {{\tiny \color{gray}.} \newline 
...
\node (DRPDBpkg) [tbox,below=3mm of DRPDB.north] {{\tiny \color{gray} \begin{verbatim} daf_ingest \end{verbatim} }  }; 
 ...
 \node (PRODEX) [pbox,below=306pt of L2SCI] {{\tiny \color{gray}.} \newline 
...
\node (WFMpkg) [tbox,below=3mm of WFM.north] {{\tiny \color{gray} \begin{verbatim} ctrl_orca/ctrl_platform_*/ctrl_execute/ctrl_stats/ctrl_provenance \end{verbatim} }  }; 
 ...
 \node (NETDM) [pbox,below=544pt of L2] {{\tiny \color{gray}.} \newline 
\textbf{DM Networks}
 }; 
...
\textbf{Long-Haul Nets}
 ...
 \node (PRECURSR) [pbox,below=340pt of NETDM] {\textbf{Precursor Data}
 }; 
\node (PRECURSRpkg) [tbox,below=3mm of PRECURSR.north] {{\tiny \color{gray} \begin{verbatim} obs_*/validation_data_*/testdata_*/afwdata \end{verbatim} }  }; 
 \draw[pline] (DM.east) -| ++(0.4,0)  |- (PRECURSR.west);
 \node (SCIALG) [pbox,below=4pt of PRECURSR] {{\tiny \color{gray}.} \newline 
...
\node (ORBEPpkg) [tbox,below=3mm of ORBEP.north] {{\tiny \color{gray} \begin{verbatim} mops_nightmops \end{verbatim} }  }; 
...
\textbf{Tracklet Gen}
 ...
 \node (PHOTOM) [pbox,below=102pt of ORBEP] {{\tiny \color{gray}1.02C.03.08} \newline 
...
\textbf{Notebook Env}
 ...
 \node (NBKSUIT) [pbox,below=4pt of NBKENV] {{\tiny \color{gray}1.02C.05.07.04} \newline 
\textbf{Notebook SUIT Intf}
 }; 
 \draw[pline] (SPLNBK.east) -| ++(0.4,0)  |- (NBKSUIT.west);
 \node (NBKSW) [pbox,below=4pt of NBKSUIT] {{\tiny \color{gray}1.02C.10.02.02.04} \newline 
\textbf{Notebook SW Env}
 }; 
 \draw[pline] (SPLNBK.east) -| ++(0.4,0)  |- (NBKSW.west);
 \node (SPLPRTL) [pbox,below=136pt of SPLNBK] {{\tiny \color{gray}.} \newline 
...
\textbf{Visualizers}
 ...
 \node (SCIPRIM) [pbox,below=374pt of SCIPLAT] {{\tiny \color{gray}.} \newline 
...
\textbf{QC Reports}
 ...
 \node (TXF) [pbox,below=102pt of SQUASH] {{\tiny \color{gray}.} \newline 
